...
### KTD4 — Place verification = the monitorability gate (existing OSM geocoder)
Every candidate place string is resolved via the existing `lib/adapters/osm.ts` (`parseGeocode` + `zoneFor`); an item that does not resolve with confidence (`reason !== "ok"` / `placeResolved === false`) is **dropped, not persisted**. The schema makes a free-text-only item *unrepresentable* (required `lat`, `lng`, `ianaZone`). Nominatim discipline: sequential ≤1 req/s, identifying `User-Agent`, cache resolved coords, geocode once at generation time and persist coords (never re-geocode on render), ODbL attribution where shown. Over-generate candidates so dropping unresolved ones doesn't leave gaps. (see Sources: Overture/GEOHALUBENCH)
...
**Approach:** Compose the **already-exported** `parseGeocode` (returns `{lat,lng,confident}`) + `zoneFor` directly — do **not** call `resolvePlace` (it forces an airport-anchored OSRM call this gate doesn't want). Keep only `confident === true` results, attaching `lat`/`lng`/`iana_zone`; map `confident === false` to the gate's own drop reason. **Rate-limiting is an explicit serial loop with a ≥1000ms spacer between un-cached Nominatim hits** (cache hits skip the spacer) — `unstable_cache` is only the cross-request memo by normalized place string; it does not enforce the within-request 1 req/s discipline. Identifying app-level `User-Agent` (a project URL/support mailbox, **never a personal email**), defined as a named constant. **Bound over-generation** (cap candidates so worst-case geocode time stays well under the 60s window). Unresolved/ambiguous candidates are dropped (logged count). Returns surviving monitorable items + a dropped-count for UI honesty.
**Patterns to follow:** `lib/adapters/osm.ts` `resolvePlace`/`AdapterResult` branching; `lib/trips/flight.ts` caching.
**Test scenarios:** resolved place → kept with coords+zone; geocode miss → dropped; ambiguous (low importance) → dropped; rate-limit from geocoder → degrade (partial result + flag), never throw; cache hit avoids a second call for the same string. `Covers R3, R6.`
...
| RK8 | **Geocoder drop-rate guts plans** — Nominatim's 0.3 importance floor drops the non-landmark places that differentiate a plan; survivors collapse to the same monuments every generic planner returns | U0 drop-rate probe *before* Phase 1; gate generate-then-verify vs retrieval-first on it; over-generate to backfill |
